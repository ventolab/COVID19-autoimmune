...
def createAnnData(folderlist, prefix, souporcell_folderlist = None, souporcell_genodico = None, autoinclude=["percent_mito", "log2p1_count", "n_genes"], min_UMI_prefilter = None, min_cell_per_gene_allowed=3, min_gene_per_cell_allowed=500, sample_obskey = "sample_names",doqcplots=False, doinspect=False, mitogeneprefix="MT-", gex_only=True,do_log2_normalize=True):
...
def binarizedDE(adata, obskey_cluster="cluster", n_gene_to_plot= 50, savepdf= "_tfidf.pdf", doinspect=False):
    ...
    #Plot TF-IDF values
    sc.pl.rank_genes_groups_dotplot(adata_bin, n_genes=5, dendrogram=False)
    ...
    if (n_gene_to_plot > 0):
        ...
        sc.pl.rank_genes_groups_dotplot(adata_count, n_genes=5, dendrogram=False, save=savepdf)
    return(adata_bin.uns['rank_genes_groups'])
...
def doLogisticRegression(adata, adata_ref, annotation_src, annotation_trg = None, annotation_trg_prob = None, cell_filter_trg = None, cell_filter_src = None, regr_max_iter = 10000, use_variable_genes=True, use_variable_genes_within_subset=True,  cc_gene_filter_prefix= None, use_raw_data=True, use_max_normalization=True, do_compute_cosine_distance=True, filtered_label = "filtered", make_circular_coords=False, cosine_coor_softmax_coef= None, logistic_coor_softmax_coef = None, C_parameter = 1, genespeficityfraction=None, doinspect=False):
    ...
    if do_compute_cosine_distance is True:
        ...
        adata_mat = anndata.AnnData(adata.raw.X, obs= adata.obs,var=adata.raw.var)[:,common_genes].X
